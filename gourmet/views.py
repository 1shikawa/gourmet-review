...
def get_keyid():
    return '9efed5d2a48021a0404880bbb3f8a8df'

class IndexView(TemplateView):
    template_name = 'gourmet/index.html'
...
def Search(request):
    ...
    return render(request, 'gourmet/search.html', params)
...
def ShopInfo(request, restid):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            # レビュー済みかの判断
            is_exist = 0
            is_exist = Review.objects.filter(shop_id=restid).filter(user=review.user).count()
            # 既に投稿済みの場合
            if not is_exist == 0:
                messages.error(request, '既にレビューを投稿済みです。')
                return redirect('gourmet:shop_info', restid)
            else:
                review.save()
            ...
            return redirect('gourmet:shop_info', restid)
        return render(request, 'gourmet/index.html', {})
    ...
    return render(request, 'gourmet/shop_info.html', params)
